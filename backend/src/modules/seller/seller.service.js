...
class SellerService {
  ...
  async accessDatabaseWithSellerId(sellerId) {
    ...
    if (errorSession)
      throw new AppError(errorSession.message, errorSession?.status || 401);

    if (!session) throw new AppError("No active session found", 401);

    if (sellerId !== session?.user?.id)
      throw new AppError("Sorry, you don't have access", 403);
  ...
  async changeRoleToSeller(userId, reqData) {
    ...
    if (errorGet)
      throw new AppError("Failed to fetch user data: " + errorGet.message, 500);
  ...
  async addProduct(productData, sellerId) {
    const { name, description, price, stock_quantity, category, img_url } =
      productData;
